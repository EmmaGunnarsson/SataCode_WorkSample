...
forvalues y = `start_year'(1)`latest_full_year' {
	...
	import delimited "$elspot\Market_coupling_capacity\20`y'\mcap`y'`w'.sdv", clear
	drop if v4==""
	...
	nrow
		...
		rename v6 Date
		save "$data\Market_coupling_capacity\Hour_Data\20`y'\mcap`y'`w'.dta", replace	
	}	
		}
...
if `current_year_latest_week' > 9 {
	...
	foreach w of local nums {	
		...
		import delimited "$elspot\Market_coupling_capacity\mcap`current_year'`w'.sdv", clear
		drop if v4==""
		...
		save "$data\Market_coupling_capacity\Hour_Data\20`current_year'\mcap`current_year'`w'.dta", replace
	}		
}		
...
forvalues y = `start_year'(1)`latest_full_year' {
		...
		use "$data\Market_coupling_capacity\Hour_Data\20`y'\mcap`y'01.dta", clear
		
		foreach w of local weeks {	
			append using "$data\Market_coupling_capacity\Hour_Data\20`y'\mcap`y'`w'.dta"
	}
		di "`y'"
		tab Week	//week 34 ?
		tab Day
		...
		tab __Data_type
		tab Code
		save "$data\Market_coupling_capacity\mcap_`y'", replace
}	
foreach y of local years_with_53_weeks {	
		numlist "10(1)53"
		local weeks 02 03 04 05 06 07 08 09 `r(numlist)'
...
//Current year
if `current_year_latest_week' > 9 {
	use "$data\Market_coupling_capacity\Hour_Data\20`current_year'\mcap`current_year'01.dta", clear
		numlist "10(1)`current_year_latest_week'"
		local weeks 02 03 04 05 06 07 08 09 `r(numlist)'
			foreach w of local weeks {		
				append using "$data\Market_coupling_capacity\Hour_Data\20`current_year'\mcap`current_year'`w'.dta"
			}
		drop __Data_type Code
		save "$data\Market_coupling_capacity\mcap_`current_year'", replace
}
if `current_year_latest_week' < 10  {
		foreach w of local current_year_available_weeks {	
			if `w'==01 {
...
//Append years
	use "$data\Market_coupling_capacity\mcap_`start_year'", clear
	local app_start `=`start_year'+1' 
	forvalues y = `app_start'(1)`current_year' {
		append using "$data\Market_coupling_capacity\mcap_`y'.dta"
		erase "$data\Market_coupling_capacity\mcap_`y'.dta"
	}
		erase "$data\Market_coupling_capacity\mcap_`start_year'.dta"
...
//Formating, correcting the Year-variable, sorting and more
	...
	order __Data_type Code Year Week Day Date edate Alias Hour 
	sort  Year __Data_type Code  edate Alias Hour
	
	sort Year edate Alias Hour

	save "$data\Market_coupling_capacity\Year_Data\Market_coupling_capacity_y`start_year'_y`current_year'.dta", replace
